...
class Equation:
    ...
    def Euler(self,f_list):
      ...
      for j in range(len(data.x)-1):
       ...
       print("Euler percentage: %s" % (j/(len(data.x)-1)*100))
      return y
    ...
    def Heun(self,f_list):
      ...
      for j in range(len(data.x)-1):
       ...
       print("Heun percentage: %s" % (j/(len(data.x)-1)*100))
      return y
...
if __name__ == '__main__':
     eq=Equation()
     f1=eq.array_to_sympy(0)
     f2=eq.array_to_sympy(1)
     f3=eq.array_to_sympy(2)
     f4=eq.array_to_sympy(3)
     f_list=[f1,f2,f3,f4]
     euler_result=eq.Euler(f_list)
     Heun_result=eq.Heun(f_list)
