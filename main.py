...
class Equation:
    ...
    def equation_solver_euler(self, x1, y, z, h):
        n = int((z - x1) / h)
        m = len(y)
        lgx = []
        lgy = []
        ...
        for i in range(0, n + 1):
            lgx.append(round(x1, 3))
            lgy.append(y[0])

            for j in range(0, m):
                k1[j] = h * self.rk_functions(x1, y)[j]
            for j in range(0, m):
                y[j] += k1[j]
            x1 = x1 + h
    ...
    def equation_solver_rungekutta(self, x1, y, z, h):
        n = float((z - x1) / h)
        m = len(y)
        lgx = []
        lgy = []
        ...
        for i in range(0, n + 1):
            ...
            for j in range(0, m):
                k1[j] = h * self.rk_functions(x1, y)[j]
                k2[j] = h * self.rk_functions(x1 + 0.5 * h, holder(y, k1, 0.5))[j]
                k3[j] = h * self.rk_functions(x1 + 0.5 * h, holder(y, k2, 0.5))[j]
                k4[j] = h * self.rk_functions(x1 + h, holder(y, k3, 1))[j]
